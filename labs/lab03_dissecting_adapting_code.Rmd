...
of counts of calls by months that included rows with zeros. We ultimately did that
...
couple of issues with the way we've tackled that problem. First, the logic of the 
...
able to have more than one copy of the same letter in the vector we're
...
dataframe of `dat` as the person who posted the question. 
...
Let's work from the inside out for the function call. The innermost call is to `factor`. 
Let's use the helpfile to see what that does (`?factor`).
...
Factors are very helpful when you have a variable that can only take one of a set of 
...
this function will count up the number of observations in each group within a dataset. 
...
let's find the part of our code that's a kluge. It's the code where we're trying
...
to force this dataset to be merged with our monthly call counts, and then we use the
