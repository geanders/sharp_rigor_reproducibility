...
You'll also find it useful to have the code that defines this function handy. It is:
...
step)
- Convert the column with the dates to a date-time class, so you can work with it in 
...
delete the columns from the data saved in the CSV, so that data will always be available
...
Now let's add the optional steps (adding columns for the month and year of the call, and
removing all the columns we're not interested in for our research question): 
...
installed that package, whether or not the the user has remembered to load the package in their
