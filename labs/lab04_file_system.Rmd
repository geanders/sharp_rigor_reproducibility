...
Here are the functions we defined in an earlier lab and lecture: 
...
We could use the function that we wrote to read in the data from each file. Already, 
this will make our code more efficient than if we wrote the code to read and clean 
...
At this point, you could run code to separately generate monthly counts of calls
for reptiles and amphibians for each borough:
...
Notice how often we're having to copy and paste this code. We could remove a lot of 
this if we bind all the data together into one big dataframe after we read it in but 
...
At this point, it's very easy to use the data to create a plot to visualize the 
results: 
...
We can use this call to create a vector with the filenames of the files for all boroughs: 
...
borough names; one is to get rid of everything we don't want, which we can do
with the `str_remove` function from the `stringr` package. Since "Staten Island"
is two words, we'll also want to replace the underscore between Staten and
Island with a space, which we can do with `str_replace` (also from the `stringr`
package). We'll put the results in a new column called `borough`:
