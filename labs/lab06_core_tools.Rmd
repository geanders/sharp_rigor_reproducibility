...
some strategies for becoming more familiar with a tool you want to add
to your toolset. These strategies can apply more broadly as you build the set of 
...
in these examples. Let's try it with our example. First, in the `snake_examples`, 
...
the `location` column is not, and therefore it's important that we look for both 
"Playground" and "playground". 
...
In these tasks, we've dug into how one tool in `stringr` works with regular expressions, 
